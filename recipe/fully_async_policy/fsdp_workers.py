...
class DetachNcclSync(AsyncActorRolloutRefWorker):
    ...
    def sync_rollout_weights(self):
        ...
        # Broadcast and load weights
        for key, shape, dtype in self._weights_info:
            tensor = torch.empty(shape, dtype=dtype, device=get_torch_device().current_device())
            if self._is_actor:
                assert key in params, f"Key {key} not found in actor params. Available keys: {list(params.keys())}"
...
class DetachActorWorker(DetachNcclSync):
    """Actor worker for fully async training with LoRA support"""
    ...
    def _get_actor_params(self):
        """Get actor parameters including LoRA weights"""
        assert self._is_actor
        
        # Check if model has LoRA
        peft_model = getattr(self.actor_module_fsdp, "_fsdp_wrapped_module", self.actor_module_fsdp)
        if hasattr(peft_model, "peft_config"):
            # Use LoRA-aware parameter collection
            params = collect_lora_params(
                module=self.actor_module_fsdp,
                layered_summon=False,
                base_sync_done=False,
            )
            # Replace wrapper names for base layer compatibility
            peft_config = peft_model.peft_config.get("default", None)
            if peft_config is not None:
                params = {replace_lora_wrapper(k, peft_config): v for k, v in params.items()}
        else:
            # Use standard state_dict if no LoRA
