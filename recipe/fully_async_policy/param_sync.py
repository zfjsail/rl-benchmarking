...
class ParameterSynchronizer:
    ...
    def _init_weights_info(self):
        """Initialize weights info from actor worker (automatically includes LoRA parameters)"""
        self.weights_info = self.actor_wg.get_actor_weights_info()[0]
        # Log LoRA info if present
        lora_param_count = sum(1 for key, _, _ in self.weights_info if "lora_" in key)
        if lora_param_count > 0:
            logger.info(f"[ParameterSynchronizer] LoRA enabled: {lora_param_count} LoRA parameters detected")
