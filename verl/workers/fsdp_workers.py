...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        if self._is_lora:
            ...
            # FIX: Ensure LoRA parameters are in correct dtype after application
            # LoRA parameters default to float32, but we need them in torch_dtype for FSDP
...
class CriticWorker(Worker, DistProfilerExtension):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        if self._is_lora:
            ...
            # FIX: Ensure LoRA parameters are in correct dtype after application
            # LoRA parameters default to float32, but we need them in torch_dtype for FSDP
            critic_module = critic_module.to(torch_dtype)
