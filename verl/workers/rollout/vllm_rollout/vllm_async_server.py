...
from verl.workers.rollout.replica import RolloutMode, RolloutReplica, TokenOutputwithTurnLogprobs
...
class vLLMHttpServerBase:
    ...
    async def generate(
    ...
    ) -> TokenOutputwithTurnLogprobs:
        """Generate sequence with token-in-token-out."""
        # TODO(@wuxibin): switch to `/generate` http endpoint once multi-modal support ready.
        max_tokens = self.config.max_model_len - len(prompt_ids)

        #TODO 这里需要修改，logprobs需要设置为2，用于计算Yes/No的logit
        sampling_params["logprobs"] = 1000
        sampling_params.setdefault("repetition_penalty", self.config.get("repetition_penalty", 1.0))
        sampling_params = SamplingParams(max_tokens=1, **sampling_params)
        ...
        return TokenOutputwithTurnLogprobs(token_ids=token_ids, log_probs=log_probs, turn_logprobs=final_res.outputs[0].logprobs[0])
