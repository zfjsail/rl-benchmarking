...
# 新增：配置 reward 类型，默认为 'ce'（Cross-Entropy），可设置为 'cls'（Classification）
REWARD_TYPE=${REWARD_TYPE:-ce}

python3 -m verl.trainer.main_ppo \
    algorithm.adv_estimator=grpo \
    data.train_batch_size=$TRAIN_BATCH_SIZE \
    trainer.default_local_dir=outputs/multiturn_grpo_test \
    ...
    'actor_rollout_ref.rollout.multi_turn.interaction_kwargs={reward_type: '"'"'$REWARD_TYPE'"'"'}' \
    actor_rollout_ref.rollout.agent.default_agent_loop='tool_agent' \
    actor_rollout_ref.rollout.calculate_log_probs=True \
    actor_rollout_ref.rollout.enable_prefix_caching=True \
    actor_rollout_ref.model.path=/workspace/pangyunhe/models/custom/qwen3-8b-lora \
